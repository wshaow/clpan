...
def parse(opt_path):
    ...
    4. set datasets parameters  phase  scale rgb_range
    5. add network para scale
    6. add a new big para path
    7. consider finetune and resume condition
    8. backup before exp result
    :param opt_path:
    :return:
    """
    ...
    config_str = f'{network_opt["which_model"].upper()}_in{network_opt["in_channels"]}_x{opt["scale"]}'
    ...
    if opt['is_train'] and opt['solver']['pretrain']:
        ...
        exp_path = os.path.dirname(os.path.dirname(
            opt['solver']['pretrained_path']))  # ref https://appdividend.com/2022/01/29/python-os-path-dirname/
